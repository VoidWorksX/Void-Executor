--!native
--!optimize 2
local VoidWorkX_UNIQUE = "%VoidWorkX_UNIQUE_ID%"
...
if CoreGui:FindFirstChild("VoidWorkX") then return end

local VoidWorkXContainer = Instance.new("Folder", CoreGui)
VoidWorkXContainer.Name = "VoidWorkX"
local objectPointerContainer, scriptsContainer = Instance.new("Folder", VoidWorkXContainer), Instance.new("Folder", VoidWorkXContainer)
...
local VoidWorkX = {
	about = {
		_name = 'VoidWorkX',
		_version = '%VoidWorkX_VERSION%'
	}
}
table.freeze(VoidWorkX.about)
...
shared.VoidWorkX = VoidWorkX -- unprotected for sharing across all scripts (easily detected)

local libs = {
	{
		['name'] = "HashLib",
		['url'] = "https://rizve.us.to/VoidWorkX/hash"
	},
	{
		['name'] = "lz4",
		['url'] = "https://rizve.us.to/VoidWorkX/lz4"
	},
	{
		['name'] = "DrawingLib",
		['url'] = "https://rizve.us.to/VoidWorkX/drawing"
	}
}

if script.Name == "VRNavigation" then
	print("[VoidWorkX]: Used ingame method. When you leave the game it might crash!")
...
function Bridge:SyncFiles()
	...
	local success = pcall(function()
		getAllFiles("./")
	end) if not success then print("[VoidWorkX]: Could not sync virtual files from client to external. Server was closed or it is being overloaded") return end
	local latestSave = {}
...
function Bridge:loadstring(source, chunkName)
	...
	coreModule.Parent = VoidWorkXContainer
	...
	if result then
		...
		while task.wait() do
			...
			if (tick() - clock > 5) then
				warn("[VoidWorkX]: loadstring failed and timed out")
				for _, module in pairs(cachedModules) do
					module:Destroy()
				end
...
function Bridge:request(options)
	...
	return {
		Success = false,
		StatusMessage = "Can't connect to VoidWorkX web server: " .. self.serverUrl,
		StatusCode = 599;
...
VoidWorkX.VoidWorkX = {
	PID = ProcessID,
	GUID = VoidWorkX_UNIQUE,
	HttpSpy = function(state)
		if state == nil then state = true end
		assert(type(state) == "boolean", "invalid argument #1 to 'HttpSpy' (boolean expected, got " .. type(state) .. ") ", 2)
		VoidWorkX.rconsoleinfo("Http Spy is set to '" .. tostring(state) .. "'")
		httpSpy = state
	end,
}

function VoidWorkX.VoidWorkX.get_real_address(instance)
...
function VoidWorkX.VoidWorkX.spoof_instance(instance, newinstance)
	...
	do
		if type(newinstance) == "number" then 
			...
			newAddress = VoidWorkX.VoidWorkX.get_real_address(newinstance)
		end
	end
	local objectValue = Instance.new("ObjectValue", objectPointerContainer)
	objectValue.Name = HttpService:GenerateGUID(false)
...
function VoidWorkX.VoidWorkX.GetGlobal(global_name)
...
function VoidWorkX.VoidWorkX.SetGlobal(global_name, value)
...
function VoidWorkX.loadstring(source, chunkName)
...
function VoidWorkX.request(options)
	...
	if httpSpy then
		VoidWorkX.rconsoleprint("-----------------[VoidWorkX Http Spy]---------------\nUrl: " .. options.Url .. 
	...
	options.Headers["User-Agent"] = options.Headers["User-Agent"] or "VoidWorkX/" .. tostring(VoidWorkX.about._version)
	options.Headers["Exploit-Guid"] = tostring(hwid)
	options.Headers["VoidWorkX-Fingerprint"] = tostring(hwid)
...
VoidWorkX.http = {request = VoidWorkX.request}
VoidWorkX.http_request = VoidWorkX.request

function VoidWorkX.HttpGet(url, returnRaw)
	assert(type(url) == "string", "invalid argument #1 to 'HttpGet' (string expected, got " .. type(url) .. ") ", 2)
	local returnRaw = returnRaw or true

	local result = VoidWorkX.request({
		Url = url,
		Method = "GET"
	})
...
function VoidWorkX.getgenv()
	return shared.VoidWorkX
end
...
function VoidWorkX.readfile(path)
...
function VoidWorkX.writefile(path, content)
...
function VoidWorkX.appendfile(path, content)
	...
	if readVal then
		return VoidWorkX.writefile(path, readVal .. content)
	end
end
function VoidWorkX.loadfile(path)
	assert(type(path) == "string", "invalid argument #1 to 'loadfile' (string expected, got " .. type(path) .. ") ", 2)
	return VoidWorkX.loadstring(VoidWorkX.readfile(path))
end
VoidWorkX.dofile = VoidWorkX.loadfile
function VoidWorkX.isfolder(path)
...
function VoidWorkX.isfile(path) -- return not VoidWorkX.isfolder(path)
...
function VoidWorkX.listfiles(path)
...
function VoidWorkX.makefolder(path)
...
function VoidWorkX.delfolder(path)
...
function VoidWorkX.delfile(path)
...
for i, libInfo in pairs(libs) do
	task.spawn(function()
		...
		--print("[VoidWorkX]: Successfully loaded library:", libInfo.name)
		libsLoaded += 1
	end)
end
...
VoidWorkX.base64 = base64
VoidWorkX.base64_encode = base64.encode
VoidWorkX.base64_decode = base64.decode
...
VoidWorkX.crypt.generatebytes = function(len)
	return VoidWorkX.crypt.generatekey(len)
end
VoidWorkX.crypt.random = function(len)
	return VoidWorkX.crypt.generatekey(len)
end
VoidWorkX.crypt.decrypt = VoidWorkX.crypt.encrypt

function VoidWorkX.crypt.hash(txt, hashName)
...
VoidWorkX.hash = VoidWorkX.crypt.hash

VoidWorkX.crypt.lz4 = lz4
VoidWorkX.crypt.lz4compress = lz4.compress
VoidWorkX.crypt.lz4decompress = lz4.decompress

VoidWorkX.lz4 = lz4
VoidWorkX.lz4compress = lz4.compress
VoidWorkX.lz4decompress = lz4.decompress

local Drawing, drawingFunctions = DrawingLib.Drawing, DrawingLib.functions
VoidWorkX.Drawing = Drawing
...
-- / Miscellaneous \ --
function VoidWorkX.saveinstance(options)
	options = options or {}
	...
	return VoidWorkX.loadstring(VoidWorkX.HttpGet("https://raw.githubusercontent.com/luau/SynSaveInstance/main/saveinstance.luau", true), "saveinstance")()(options)
end
VoidWorkX.savegame = VoidWorkX.saveinstance

function VoidWorkX.getexecutorname()
	return VoidWorkX.about._name
end
function VoidWorkX.getexecutorversion()
	return VoidWorkX.about._version
end

function VoidWorkX.identifyexecutor()
	return VoidWorkX.getexecutorname(), VoidWorkX.getexecutorversion()
end
VoidWorkX.whatexecutor = VoidWorkX.identifyexecutor

function VoidWorkX.get_hwid()
	return hwid
end
VoidWorkX.gethwid = VoidWorkX.get_hwid

function VoidWorkX.getscriptbytecode(script_instance)
...
VoidWorkX.dumpstring = VoidWorkX.getscriptbytecode

function VoidWorkX.queue_on_teleport(source)
...
VoidWorkX.queueonteleport = VoidWorkX.queue_on_teleport

function VoidWorkX.setclipboard(content)
...
VoidWorkX.toclipboard = VoidWorkX.setclipboard

function VoidWorkX.rconsoleclear()
	return Bridge:rconsole("cls")
end
VoidWorkX.consoleclear = VoidWorkX.rconsoleclear

function VoidWorkX.rconsolecreate()
	return Bridge:rconsole("crt")
end
VoidWorkX.consolecreate = VoidWorkX.rconsolecreate

function VoidWorkX.rconsoledestroy()
	return Bridge:rconsole("dst")
end
VoidWorkX.consoledestroy = VoidWorkX.rconsoledestroy

function VoidWorkX.rconsoleprint(text)
...
VoidWorkX.consoleprint = VoidWorkX.rconsoleprint

function VoidWorkX.rconsoleinfo(text)
...
VoidWorkX.consoleinfo = VoidWorkX.rconsoleinfo

function VoidWorkX.rconsolewarn(text)
	assert(type(text) == "string", "invalid argument #1 to 'rconsolewarn' (string expected, got " .. type(text) .. ") ", 2)
	return Bridge:rconsole("prt", "[!] " .. text)
end
VoidWorkX.consolewarn = VoidWorkX.rconsolewarn

function VoidWorkX.rconsolesettitle(text)
...
VoidWorkX.rconsolename = VoidWorkX.rconsolesettitle
VoidWorkX.consolesettitle = VoidWorkX.rconsolesettitle
VoidWorkX.consolename = VoidWorkX.rconsolesettitle

function VoidWorkX.clonefunction(func)
...
function VoidWorkX.islclosure(func)
...
function VoidWorkX.fireclickdetector(part)
...
function VoidWorkX.firetouchinterest(toucher, toTouch, touch_state)
	...
	local toTouchAddress = tostring(VoidWorkX.VoidWorkX.get_real_address(toTouch))
	...
	if touch_state == 0 then
		...
		VoidWorkX.VoidWorkX.spoof_instance(newPart, toTouch)
	...
	elseif touch_state == 1 then
		...
		VoidWorkX.VoidWorkX.spoof_instance(toTouch, tonumber(toTouchAddress))
...
function VoidWorkX.fireproximityprompt(proximityprompt, amount, skip)
...
function VoidWorkX.setsimulationradius(newRadius, newMaxRadius)
...
function VoidWorkX.isreadonly(t)
...
function VoidWorkX.setreadonly(t, state)
...
function VoidWorkX.rconsoleinput(text)
	task.wait()
	return "N/A"
end
VoidWorkX.consoleinput = VoidWorkX.rconsoleinput

function VoidWorkX.getrenv()
	return {
...
function VoidWorkX.isexecutorclosure(func)
	...
	if check(VoidWorkX.getgenv().getrenv()) then
		return false
	end
	return true
end
VoidWorkX.checkclosure = VoidWorkX.isexecutorclosure
VoidWorkX.isourclosure = VoidWorkX.isexecutorclosure
...
function VoidWorkX.isrbxactive()
	return windowActive
end
VoidWorkX.isgameactive = VoidWorkX.isrbxactive
VoidWorkX.iswindowactive = VoidWorkX.isrbxactive

function VoidWorkX.getinstances()
	return workspace.Parent:GetDescendants()
end

local nilinstances, cache = {Instance.new("Part")}, {cached = {}}

function VoidWorkX.getnilinstances()
	return nilinstances
end
...
VoidWorkX.cache = cache

function VoidWorkX.getgc()
...
function VoidWorkX.getrunningscripts()
	local scripts = {}
	for _, v in pairs(VoidWorkX.getinstances()) do
		if v:IsA("LocalScript") and v.Enabled then table.insert(scripts, v) end
	end
	return scripts
end
VoidWorkX.getscripts = VoidWorkX.getrunningscripts

function VoidWorkX.getloadedmodules()
	local modules = {}
	for _, v in pairs(VoidWorkX.getinstances()) do
...
function VoidWorkX.checkcaller()
	local info = debug.info(VoidWorkX.getgenv, 'slnaf')
	return debug.info(1, 'slnaf')==info
end

function VoidWorkX.getthreadcontext()
	return 3
end
VoidWorkX.getthreadidentity = VoidWorkX.getthreadcontext
VoidWorkX.getidentity = VoidWorkX.getthreadcontext

function VoidWorkX.setthreadidentity()
	return 3, "Not Implemented"
end
VoidWorkX.setidentity = VoidWorkX.setthreadidentity
VoidWorkX.setthreadcontext = VoidWorkX.setthreadidentity

function VoidWorkX.getsenv(script_instance)
	local env = getfenv(2)
...
function VoidWorkX.getscripthash(instance) -- !
...
function VoidWorkX.getconnections(event)
...
function VoidWorkX.hookfunction(func, rep) -- hooks global function only
	local env = getfenv(2)
...
VoidWorkX.replaceclosure = VoidWorkX.hookfunction

function VoidWorkX.cloneref(a)
	local s, x = pcall(function() return workspace.Parent.Clone(a) end) return s and x or a
end

function VoidWorkX.gethui()
	return VoidWorkX.cloneref(workspace.Parent:FindService("CoreGui"))
end

function VoidWorkX.isnetworkowner(part)
...
VoidWorkX.debug = table.clone(debug) -- the debug funcs was not by me (.rizve) credits goes to the person that made it
function VoidWorkX.debug.getinfo(f, options)
...
function VoidWorkX.debug.getmetatable(table_or_userdata)
...
VoidWorkX.debug.setmetatable = setmetatable

function VoidWorkX.getrawmetatable(object)
	...
	local raw_mt = VoidWorkX.debug.getmetatable(object)
	if raw_mt and raw_mt.__metatable then
		raw_mt.__metatable = nil 
		local result_mt = VoidWorkX.debug.getmetatable(object)
...
function VoidWorkX.setrawmetatable(object, newmetatbl)
...
function VoidWorkX.hookmetamethod(t, index, func)
	...
	local o = t
	local mt = VoidWorkX.debug.getmetatable(t)
	mt[index] = func
	t = mt
	return o
end
...
function VoidWorkX.setfpscap(cap)
	cap = tonumber(cap)
