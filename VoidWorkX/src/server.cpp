...
static void serve(Response& res, json& body) {
	...
	if (cType == "ax") { // get autoexec contents
		std::string content;
		std::filesystem::path VoidWorkXDir = std::filesystem::current_path().parent_path();
		std::filesystem::path autoexecDir = VoidWorkXDir / "autoexec";
