    ...
    PID(processID)
{
    ...
    if (CoreGui->FindFirstChild("VoidWorkX")) {
    ...
    replaceString(clientScript, "%VoidWorkX_UNIQUE_ID%", GUID);
    replaceString(clientScript, "%VoidWorkX_VERSION%", "1.0.0");
...
void RBXClient::execute(const std::string& source) const {
    ...
    Instance* VoidWorkXFolder = CoreGui->FindFirstChild("VoidWorkX");
    if (!VoidWorkXFolder)
        return;

    Instance* VoidWorkXModules = VoidWorkXFolder->FindFirstChild("Scripts");
    if (!VoidWorkXModules)
        return;

    Instance* VoidWorkXModule = VoidWorkXModules->FindFirstChildOfClass("ModuleScript");
    if (!VoidWorkXModule)
        return;

    VoidWorkXModule->SetBytecode(Compile("return{[\"x e n o\"]=function(...)function c(x,y)z=x;for i,v in y do z[i]=v;end;return z;end;setfenv(1,c(getfenv(1),shared.VoidWorkX))setfenv(0,c(getfenv(0),shared.VoidWorkX))for i,f in shared.VoidWorkX do getfenv(0)[i] = f;getfenv(1)[i]=f;end;setmetatable(shared.VoidWorkX,{__newindex=function(t,i,v)rawset(t,i,v);for i,v in t do getfenv()[i]=v;end;end,__index=function(t,val)return rawget(t,val);end});" + source + "\nend};"), true);
    VoidWorkXModule->UnlockModule();
}
...
bool RBXClient::loadstring(const std::string& source, std::string& script_name, std::string& chunk_name) const {
    ...
    Instance* cloned_module = VoidWorkXFolder->FindFirstChild(script_name);
    if (cloned_module->Self() == 0)
        return false;

    cloned_module->SetBytecode(Compile("return{[ [[" + chunk_name + "]] ] = function(...)function c(x,y)z=x;for i,v in y do z[i]=v;end;return z;end;setfenv(1,c(getfenv(1),shared.VoidWorkX))setfenv(0,c(getfenv(0),shared.VoidWorkX))for i,f in shared.VoidWorkX do getfenv(0)[i] = f;getfenv(1)[i]=f;end;setmetatable(shared.VoidWorkX,{__newindex=function(t,i,v)rawset(t,i,v);for i,v in t do getfenv()[i]=v;end;end,__index=function(t,val)return rawget(t,val);end});" + source + "\nend};"), true);
...
std::uintptr_t RBXClient::GetObjectValuePtr(const std::string& objectval_name) const
{
    ...
    Instance* objectValContainer = VoidWorkXFolder->FindFirstChild("Instance Pointers");
